...
// Map: longest-prefix-match trie of CIDRs → allow/deny flag (u8)
struct {
    __uint(type,        BPF_MAP_TYPE_LPM_TRIE);
    __uint(max_entries, 700000);               // tweak if you load more CIDRs
    __uint(map_flags,   BPF_F_NO_PREALLOC);
    __type(key,         struct lpm_key);
    __type(value,       __u8);
} geoip_trie_map SEC(".maps");

struct {
    __uint(type, BPF_MAP_TYPE_LRU_HASH);
    __uint(max_entries, 100000);
    __type(key, __u32);   // Source IP address
    __type(value, __u32);  // Tcp connection count.
...
static __always_inline __u8 is_geo_allowed(__u32 ip) {
    volatile struct geo_config *geo_config_val = &global_fw_config.g_geo_config;
    if (!geo_config_val->check)
        return 1;
    ...
    __u8 *allowed = bpf_map_lookup_elem(&geoip_trie_map, &key);
...
int xdp_parse_syn(struct xdp_md *ctx) {
    ...
    if (config->valid == 0) {
        if (!is_geo_allowed(src_ip)) return XDP_DROP;
